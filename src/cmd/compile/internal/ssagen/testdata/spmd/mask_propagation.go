...
func testBasicMaskPropagationSSA() {
	...
	go for i := range 42 {
		var condition lanes.Varying[bool] = i > 20
...
func testNestedMaskPropagationSSA() {
	...
	go for data := range 60 {
		var outer lanes.Varying[bool] = data > 10
		var inner lanes.Varying[bool] = data < 50

		var result lanes.Varying[int32] = data
		if outer {
			// EXPECT SSA: mask = currentMask & outer
			if inner {
				// EXPECT SSA: mask = currentMask & outer & inner
				result = data * lanes.Varying[int32](3)
			} else {
				// EXPECT SSA: mask = currentMask & outer & !inner
				result = data * lanes.Varying[int32](2)
			}
		} else {
			// EXPECT SSA: mask = currentMask & !outer
			result = data + lanes.Varying[int32](5)
		}
...
func testSwitchMaskPropagationSSA() {
	...
	go for data := range 30 {
		var selector lanes.Varying[int32] = data % 3
...
func testReduceMaskPropagationSSA() {
	...
	var condition lanes.Varying[bool] = data > lanes.Varying[int32](20)
	...
	if reduce.All(condition) {
		...
		// EXPECT SSA: uniform branch, no mask needed
		var result lanes.Varying[int32] = data * lanes.Varying[int32](2)
		process(result)
	} else {
		// EXPECT SSA: uniform branch, no mask needed
		process(0)
...
func testGoForMaskPropagationSSA() {
	...
	go for i := range 16 {
		var condition1 lanes.Varying[bool] = i > lanes.Varying[int32](4)
		var condition2 lanes.Varying[bool] = i < lanes.Varying[int32](12)
		var combinedCond lanes.Varying[bool] = condition1 && condition2

		if combinedCond {
			// EXPECT SSA: mask = loopMask & combinedCond
			var data lanes.Varying[int32] = lanes.Varying[int32](i) * lanes.Varying[int32](3)
			process(data)
		}

		if condition1 {
			// EXPECT SSA: mask = loopMask & condition1
			var other lanes.Varying[int32] = lanes.Varying[int32](i) + lanes.Varying[int32](10)
			process(other)
		}
	}
}
...
func testFunctionCallMaskPropagationSSA() {
	...
	go for data := range 50 {
		var condition lanes.Varying[bool] = data > 30
...
func maskedSPMDFunction(value lanes.Varying[int32]) lanes.Varying[int32] {
...
func testSelectMaskPropagationSSA() {
	...
	var data lanes.Varying[int32] = 20
	...
	case val := <-ch:
		...
		varyingResult := data + lanes.Varying[int32](uniformResult)
		process(varyingResult)
	default:
		// EXPECT SSA: varying operations in uniform context, all lanes on
		defaultResult := data * lanes.Varying[int32](3)
