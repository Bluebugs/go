...
func processPtrSPMD(ptr lanes.Varying[*int]) lanes.Varying[int] {
	return *ptr * 2
}

// Uniform function taking uniform pointer
func processPtrUniform(ptr *int) {
	value := *ptr
	process(value)
}
...
func testPointerToVaryingTypes() {
	var validPtr *lanes.Varying[int]

	var validArray [4]*lanes.Varying[int]
