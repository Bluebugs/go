...
func (check *Checker) validateSPMDPointerOperation(x *operand, op token.Token, operandExpr ast.Expr) (handled, valid bool, errorMsg string) {
	if !buildcfg.Experiment.SPMD {
		return false, false, ""
	}

	switch op {
	case token.AND: // Address operation (&)
...
func (check *Checker) validateSPMDAddressOperation(x *operand, operandExpr ast.Expr) (handled, valid bool, errorMsg string) {
	...
	if indexExpr, ok := operandExpr.(*ast.IndexExpr); ok {
		var idx operand
		...
		if binExpr, ok := indexExpr.Index.(*ast.BinaryExpr); ok && binExpr.Op == token.MUL {
...
func (check *Checker) validateSPMDPointerArithmetic(x *operand, op token.Token) (handled, valid bool, errorMsg string) {
	if !buildcfg.Experiment.SPMD {
		return false, false, ""
	}
...
func (check *Checker) validateSPMDTypeSwitchCases(cases []ast.Expr, hasDefault bool) (bool, string) {
	if !buildcfg.Experiment.SPMD {
		return false, ""
	}
...
func (check *Checker) validateSPMDVaryingPointerAccess(x *operand, index ast.Expr) (bool, string) {
	if !buildcfg.Experiment.SPMD {
		return false, ""
	}
